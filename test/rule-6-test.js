...
tape('rule_6 marks positive trend', function(test) {
  ...
  test.ok(utils.marked(result[0], 'rule_6'), 'el 0 is marked');
  test.ok(utils.marked(result[1], 'rule_6'), 'el 1 is marked');
  test.ok(utils.marked(result[2], 'rule_6'), 'el 2 is marked');
  test.ok(utils.marked(result[4], 'rule_6'), 'el 4 is marked');
...
tape('rule_6 does not mark no variance', function(test) {
  ...
  resultAlt.forEach(function(res, i) {
    test.ok(utils.notMarked(res, 'rule_6'), 'el alt ' + i + ' is not marked');
  });

  var resultMag = rule_6(input_no_match_mag, mean, std);
  resultMag.forEach(function(res, i) {
    test.ok(utils.notMarked(res, 'rule_6'), 'el mag ' + i + ' is not marked');
  });
