...
tape('rule_5 marks positive trend', function(test) {
  ...
  test.ok(utils.marked(result[0].el, 'rule_5'), 'el 0 is marked');
  test.ok(utils.marked(result[2].el, 'rule_5'), 'el 2 is marked');
...
tape('rule_5 does not mark no variance', function(test) {
  ...
  resultAlt.forEach(function(res, i) {
    test.ok(utils.notMarked(res.el, 'rule_5'), 'el ' + i + ' is not marked');
  });
